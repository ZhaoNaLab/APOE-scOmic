...
normalize_for_heatmap <- function(x) {
# Ensure everything numeric
  x <- apply(x, MARGIN=c(1,2), function(x) type.convert(as.character(x), as.is = TRUE))

# Normalize by row minimum
  x <- x - apply(x, MARGIN=1, FUN=min)

# Normalize by row maximum
  x <- x / apply(x, MARGIN=1, FUN=max)
