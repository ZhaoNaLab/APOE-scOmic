%% MGAS_proteome_MG_WGCNA_GO_bar.m      
%% module of interest: brown, Magenta, red, green, blue, turquoise
...
%%
data_path = 'F:\APOE-TR-Omics-Data\Single cell type proteomics\Clark analysis-4th round\WGCNA\MG\minModuleSize=20,mergeCutHeight=0.25';
save_fig_path = 'F:\APOE-TR-Omics-Data\Single cell type proteomics\Clark analysis-4th round\ana fig\MG';
save_mat_path = 'F:\APOE-TR-Omics-Data\Single cell type proteomics\Clark analysis-4th round\ana mat\MG';
sig_modules = {'yellow','blue', 'brown','red'}; %% 
n_sig_modules = length(sig_modules);
MGwgxna.sigmodule = sig_modules;

%%
fn = 'MG module-GO-over-representation.xlsx';
for i = 1:n_sig_modules
    ...
    fig_fn = strcat('MG-',sig_modules(i),'-topGOterms.emf');
    cd(save_fig_path);
    saveas(gcf,char(fig_fn));
    cd(save_mat_path);
    temp = strcat('MG-',sig_modules(i),'-topGO_genelist.mat');
...
cd(save_mat_path);
    temp = strcat('MG-',sig_modules(i),'.mat');
...
e2fn = 'MG 24Mvs3ME2.xlsx';
e3fn = 'MG 24Mvs3ME3.xlsx';
e4fn = 'MG 24Mvs3ME4.xlsx';
[a2,b2,~] = xlsread(e2fn);
[a3,b3,~] = xlsread(e3fn);
[a4,b4,~] = xlsread(e4fn);
